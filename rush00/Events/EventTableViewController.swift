...
class EventTableViewController: UITableViewController, UISearchBarDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.date = filteredData[indexPath.row].beginAt
        cell.layoutIfNeeded()
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if (searchText.isEmpty) {
            ...
            filteredData = client.events.filter{String($0.campusIds[0]).range(of: result ?? "no such campus", options: .caseInsensitive) != nil}
            ...
            filteredData = client.events.filter{String($0.cursusIds[0]).range(of: result ?? "no such cursus", options: .caseInsensitive) != nil}
        } else {
            filteredData = client.events
        }
